...
public final class SEE {
	...
	/**
	 * determines the static exchange evaluation (SEE) gain or loss
	 * <p> note, this is slower than {@link #seeSign(int, long, long, State4)}, but it
	 * does determine the magnitude of gain or loss
	 * @param player
	 * @param from
	 * @param target
	 * @param s
	 * @return
	 */
	public static int see(final int player, final long from, final long target, final State4 s){
		...
		//records best gain encountered by a side
		//idea is a side may choose to stop the exchange, and will stop when up the most
		int alliedMaxGain = -9999;
		int enemyMaxGain = -9999;
		
		for(int a = 0; a < 16; a++){
			
			final int gEnemy = -gain;
			enemyMaxGain = max(enemyMaxGain, gEnemy);
			...
			final int gAllied = -gain;
			alliedMaxGain = max(alliedMaxGain, gAllied);
