...
/** profiling version*/
public final class Search33v3prof implements Search4{
	...
	public Search33v3prof(Evaluator2 e, int hashSize, boolean printPV, File logFile) throws IOException{
		this.e = e;
		...
		logger = new SearchLogger(logFile);
	}
	...
	private int recurse(final int player, int alpha, final int beta, final double depth,
		...
		int moveCount = 0;
		int quietMoveCount = 0; //number moves that dont take or check
		...
		for(int i = 0; i < length; i++){
			for(long movesTemp = moves[i]; movesTemp != 0 ; movesTemp &= movesTemp-1){
				...
				if(State4.isAttacked2(BitUtil.lsbIndex(s.kings[player]), 1-player, s)){
					...
					if(Math.random() < Math.pow(.5, 10-depth))
						logger.log(g, alpha, beta, (int)depth, quietMoveCount,
								pv, false, inCheck, isCapture, isDangerous,
								isTTEMove, MoveEncoder.getMovePieceType(encoding),
								lazyEval);
				}
				s.undoMove();
				this.e.undoMove(encoding);
