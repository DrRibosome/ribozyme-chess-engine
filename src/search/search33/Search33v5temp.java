...
public final class Search33v5temp implements Search4{
	...
	public Search33v5temp(Evaluator2 e, int hashSize){
		this(e, hashSize, false);
	}
	
	public Search33v5temp(Evaluator2 e, int hashSize, boolean printPV){
		this.e = e;
	...
	private static boolean isPseudoLegal(final int player, final long encoding, State4 s){
		...
		if((pieces[player] & p) != 0 && (pieces[player] & pieces[1-player] & m) == 0){
			...
			case State4.PIECE_TYPE_PAWN:
				...
				final long tempPawnMoves = Masks.getRawAggPawnMoves(player, pieces[0]|pieces[1], s.pawns[player]);
