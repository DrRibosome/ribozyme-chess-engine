...
public final class Search33v10 implements Search4{
	...
	public Search33v10(Evaluator2 e, int hashSize){
		this(e, hashSize, false);
	}
	
	public Search33v10(Evaluator2 e, int hashSize, boolean printPV){
		this.e = e;
	...
	public void search(final int player, final State4 s, final MoveSet moveStore, final int maxPly){
		...
		for(int i = 1; (maxPly == -1 || i <= maxPly) && !cutoffSearch.get() && i <= stackSize; i++){
			...
			score = recurse(player, alpha, beta, i, true, true, 0, s, false);
	...
	private int recurse(final int player, int alpha, final int beta, final double depth,
			final boolean pv, final boolean rootNode, final int stackIndex, final State4 s, final boolean noSEEPrune){
		stats.nodesSearched++;
		assert alpha < beta;
		...
		if(s.isForcedDraw()){
			...
			depth = 1;*/
			final int q = qsearch(player, alpha, beta, 0, stackIndex, pv, s, noSEEPrune);
			if(q > 70000 && pv){
				return recurse(player, alpha, beta, 1, true, false, stackIndex, s, true);
			} else{
				return q;
			}
		...
		if(!pv && !ml.skipNullMove && depth > 0 && !alliedKingAttacked && hasNonPawnMaterial){
			...
			int n = -recurse(1-player, -beta, -alpha, depth-r, pv, rootNode, stackIndex+1, s, noSEEPrune);
			s.undoNullMove();
			...
			if(n >= beta){
				...
				//verification search
				stack[stackIndex+1].skipNullMove = true;
				double v = recurse(player, alpha, beta, depth-r, pv, rootNode, stackIndex+1, s, noSEEPrune);
				stack[stackIndex+1].skipNullMove = false;
				if(v >= beta){
					stats.nullMoveCutoffs++;
					return n;
		...
		if(!tteMove && depth >= (pv? 5: 8) && (pv || (!alliedKingAttacked && lazyEval+256 >= beta))){
			...
			stack[stackIndex+1].skipNullMove = true;
			recurse(player, alpha, beta, d, pv, rootNode, stackIndex+1, s, noSEEPrune);
			stack[stackIndex+1].skipNullMove = false;
			final TTEntry temp;
		...
		for(int i = 0; i < length; i++){
			...
			if(State4.isAttacked2(BitUtil.lsbIndex(s.kings[player]), 1-player, s)){
				...
				if(depth > 2 && !pvMove && !isCapture && !inCheck && !isPawnPromotion &&
					...
					g = -recurse(1-player, -alpha-1, -alpha, reducedDepth, false, false, stackIndex+1, s, noSEEPrune);
					fullSearch = g > alpha;
				} else{
					fullSearch = true;
				}
				...
				if(fullSearch){
					...
					if(!pvMove){
						g = -recurse(1-player, -(alpha+1), -alpha, nextDepth, false, false, stackIndex+1, s, noSEEPrune);
						if(alpha < g && g < beta && pv){
							g = -recurse(1-player, -beta, -alpha, nextDepth, pv, false, stackIndex+1, s, noSEEPrune);
						}
					} else{
						g = -recurse(1-player, -beta, -alpha, nextDepth, pv, false, stackIndex+1, s, noSEEPrune);
					}
	...
	private int qsearch(final int player, int alpha, int beta, final int depth,
			final int stackIndex, final boolean pv, final State4 s, final boolean noSEEPrune){
		stats.nodesSearched++;
		...
		for(int i = 0; i < length; i++){
			...
			if(noSEEPrune || isPromotion || SEE.seeSign(player, pieceMask, move, s) >= 0){
				...
				if(State4.isAttacked2(BitUtil.lsbIndex(s.kings[player]), 1-player, s)){
					...
					g = -qsearch(1-player, -beta, -alpha, depth-1, stackIndex+1, pv, s, noSEEPrune);
				}
				s.undoMove();
				this.e.undoMove(encoding);
