...
public final class SearchS4V25qzitRecord implements Search3<State4>{
	public final static class SearchStat25 extends SearchStat{
	...
	private final SearchStat25 stats = new SearchStat25();
	...
	public SearchS4V25qzitRecord(int maxDepth, State4 s, Evaluator2<State4> e, int hashSize){
		this.s = s;
		this.e = e;
		m = new ZMap(hashSize);
	...
	public void search(int player, int[] moveStore, int maxPly){
		...
		for(int i = 1; (maxPly == -1 || i <= maxPly) && !cutoffSearch; i++){
			s.resetHistory();
			double alpha = i <= 3? min: score-35;
			double beta = i <= 3? max: score+35;
			if(alpha > beta){
				double temp = alpha;
				alpha = beta;
				beta = temp;
