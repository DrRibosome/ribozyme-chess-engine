...
public final class SearchS4V22qzit implements Search2<State4>{
	...
	private final SearchStat stats = new SearchStat();
	...
	public SearchS4V22qzit(int ply, State4 s, Evaluator2<State4> e, int hashSize){
		this.s = s;
		this.e = e;
		this.maxPly = ply;
		m = new ZMap(hashSize);
		stack = new MoveList[(ply+qply)*2];
		for(int i = 0; i < stack.length; i++){
			stack[i] = new MoveList();
		}
	}
	
	public SearchStat search(int player, int ply, int[] moveStore){
		...
		move[0] = bestMove;
		int pos1 = MoveEncoder.getPos1(move[0]);
		int pos2 = MoveEncoder.getPos2(move[0]);
		moveStore[0] = pos1%8;
		moveStore[1] = pos1/8;
		moveStore[2] = pos2%8;
		moveStore[3] = pos2/8;
	...
	private String getPV(int player, State4 s, String pv, int depth, int maxDepth){
		...
		if(depth < maxDepth && e != null && e.encoding != 0){
			...
			String r = getPV(1-player, s, pv, depth+1, maxDepth);
			s.undoMove();
			return r;
		}
		return pv;
	...
	@Override
	public void getMove(int[] move, int player) {
		search(player, maxPly, move);
	}
	
	@Override
	public void getMove(final int[] move, final int player, long time) {
		Thread t = new Thread(){
			public void run(){
				search(player, maxPly, move);
			}
		...
		while(t.isAlive() && System.currentTimeMillis()-start < time){
			try{
				Thread.sleep(30);
