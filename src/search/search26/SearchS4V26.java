...
public final class SearchS4V26 implements Search3<State4>{
	...
	private static void recordMoves2(int player, int pieceMovingType, long pieceMask,
		...
		if(piece != 0 && moves != 0){
			...
			int w = ml.length;
			if((moves & enemy) != 0){
				long upTakes = moves & enemy & ml.upTakes[pieceMovingType];
				for(; upTakes != 0; upTakes &= upTakes-1){
					ml.pieceMasks[w] = piece;
					ml.moves[w] = upTakes&-upTakes;
					ml.ranks[w] = 3;
					w++;
				}
	...
	private static void genMoves2(final int player, State4 s, MoveList ml, ZMap m, boolean quiece){
		...
		if(ml.kingAttacked[player]){
			...
			recordMoves2(player, State4.PIECE_TYPE_KING, s.kings[player], kingMoves, ml, s, m, false);
		}
		...
		if(!ml.kingAttacked[player]){
			...
			recordMoves2(player, State4.PIECE_TYPE_KING, s.kings[player], kingMoves, ml, s, m, quiece);
		}
	}
