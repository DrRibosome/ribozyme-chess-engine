...
public final class SearchS4V34 implements Search3{
	...
	private final static class MoveList{
		...
		/** takes that take upwards*/
		public final long[] upTakes = new long[7];
		/** takes on even material*/
	...
	public SearchS4V34(State4 s, Evaluator2<State4> e, int hashSize, boolean record){
		this.s = s;
		this.e = e;
		m = new ZMap(hashSize);
		stack = new MoveList[stackSize];
		for(int i = 0; i < stack.length; i++){
			stack[i] = new MoveList(i);
		}
		...
		if(record){
			try{
				f = new FileWriter("search33.stats", true);
	...
	public void search(int player, int[] moveStore, int maxPly){
		...
		if(f != null){
			try{
				f.write(StatsStore.recordPVScores(s, player, maxPlySearched, stats.scores)+"\n");
				f.write(StatsStore.recordBranchingFactor(stats.empBranchingFactor)+"\n");
				f.flush();
			} catch(IOException a){
				a.printStackTrace();
			}
		...
		for(int i = 0; i < length && !cutoffSearch; i++){
			for(long movesTemp = moves[i]; movesTemp != 0 ; movesTemp &= movesTemp-1){
				...
				if(State4.isAttacked2(BitUtil.lsbIndex(s.kings[player]), 1-player, s)){
					...
					//System.out.println("in check after move, "+stackIndex+"\n"+s);
				} else{
					...
					if(1==2&&depth > 2 && !pvMove && !isCapture && !givesCheck){
						int reducedDepth = i > 2? (pv? depth-2: depth/2): (pv? depth-1: depth-2);
				...
				if(alpha >= beta){
					//System.out.println("stack = "+stackIndex+", beta cutoff, beta="+beta+", alpha = "+alpha);
					if(!cutoffSearch){
						m.put2(s.zkey(), bestMove, alpha, depth, ZMap.CUTOFF_TYPE_LOWER);
						
						if(stackIndex-2 >= 0 &&
								MoveEncoder.getTakenType(bestMove) == 0 &&
								!MoveEncoder.isPawnPromoted(bestMove)){
							MoveList temp = stack[stackIndex-2];
							final long[] k = temp.killer;
							if(k[0] != bestMove && k[1] != bestMove){
								k[1] = k[0];
								k[0] = bestMove;
							}
						}
	...
	private void recordMoves(int player, int pieceMovingType, long pieceMask,
			long moves, MoveList ml, long[] killer, State4 s, ZMap m, boolean quiesce){
		...
		if(piece != 0 && moves != 0){
			...
			if(takes != 0){
				...
				final long downTakes = takes & ~(upTakes | trades);
				if(downTakes != 0){
					ml.pieceMasks[w] = piece;
			...
			long nonTake = moves & ~enemy;
			if(nonTake != 0 && !quiesce){
				//final long[] k = stack[stackI]
				if(killer != null){
					for(int a = 0; a < 2; a++){
						if(killer[a] != 0 &&
								1L<<MoveEncoder.getPos1(killer[a]) == pieceMask){
							final long kmove = 1L<<MoveEncoder.getPos2(killer[a]);
							if((kmove & nonTake) != 0){
								ml.pieceMasks[w] = piece;
								ml.moves[w] = kmove;
								ml.ranks[w] = 5;
				...
				/*for(; nonTake != 0; nonTake &= nonTake-1){
					ml.pieceMasks[w] = piece;
					final long move = nonTake & -nonTake;
					ml.moves[w] = move;
					ml.ranks[w] = 6;
					s.executeMove(player, piece, move);
					if(m.get(s.zkey()) != null){
						ml.ranks[w] = 4;
					}
					s.undoMove();
					w++;
	...
	private void recordMoves2(int player, int pieceMovingType, long pieceMask,
			long moves, MoveList ml, long[] killer, State4 s, ZMap map, boolean quiesce){
		if(moves != 0){
			final long piece = pieceMask & -pieceMask;
			if(!quiesce){
				int w = ml.length;
				for(long m = moves; m != 0; m &= m-1){
					ml.pieceMasks[w] = piece;
					ml.moves[w] = m;
					s.executeMove(player, piece, m);
					if(map.get(s.zkey()) != null){
						ml.ranks[w] = 0;
					} else{
						if((m & ml.upTakes[pieceMovingType]) != 0){
							ml.ranks[w] = 1;
						} else if((m & ml.trades[pieceMovingType]) != 0){
							ml.ranks[w] = 2;
							w++;
						} else{
							ml.ranks[w] = 3;
	...
	private void genMoves2(final int player, State4 s, MoveList ml, ZMap m, boolean quiesce){
		ml.upTakes[State4.PIECE_TYPE_KING] = s.pieces[1-player];
		ml.upTakes[State4.PIECE_TYPE_QUEEN] = 0;
		ml.upTakes[State4.PIECE_TYPE_ROOK] = s.queens[1-player];
		...
		ml.trades[State4.PIECE_TYPE_KNIGHT] = s.knights[1-player]|s.bishops[1-player];
		ml.trades[State4.PIECE_TYPE_BISHOP] = ml.upTakes[State4.PIECE_TYPE_KNIGHT];
		ml.trades[State4.PIECE_TYPE_PAWN] = s.pawns[1-player];
		
		final long[] pieces = s.pieces;
		final long[] killer = ml.stackIndex-2 < 0? null: stack[ml.stackIndex-2].killer;
		
		if(ml.kingAttacked[player]){
			recordMoves2(player, State4.PIECE_TYPE_KING, s.kings[player],
					State4.getKingMoves(player, pieces, s.kings[player]), ml, killer, s, m, quiesce);
		}

		for(long queens = s.queens[player]; queens != 0; queens &= queens-1){
			final long moves = State4.getQueenMoves(player, pieces, queens);
			recordMoves2(player, State4.PIECE_TYPE_QUEEN, queens, moves, ml, killer, s, m, quiesce);
		}
		
		for(long knights = s.knights[player]; knights != 0; knights &= knights-1){
			final long moves = State4.getKnightMoves(player, pieces, knights);
			recordMoves2(player, State4.PIECE_TYPE_KNIGHT, knights, moves, ml, killer, s, m, quiesce);
		}
		
		for(long bishops = s.bishops[player]; bishops != 0; bishops &= bishops-1){
			final long moves = State4.getBishopMoves(player, pieces, bishops);
			recordMoves2(player, State4.PIECE_TYPE_BISHOP, bishops, moves, ml, killer, s, m, quiesce);
		}
		
		for(long rooks = s.rooks[player]; rooks != 0; rooks &= rooks-1){
			final long moves = State4.getRookMoves(player, pieces, rooks);
			recordMoves2(player, State4.PIECE_TYPE_ROOK, rooks, moves, ml, killer, s, m, quiesce);
		}
		...
		int w = ml.length;
		final long[] pieceMasks = ml.pieceMasks;
		...
		for(int i = 0; i < 2; i++){
			for(long tempPawnMoves = pawnMoves[i]; tempPawnMoves != 0; tempPawnMoves&=tempPawnMoves-1){
				...
				pieceMasks[w] = pawnMask;
				moves[w] = moveMask;
				ranks[w] = (moveMask & Masks.pawnPromotionMask[player]) == 0? 3: 0;
				w++;
			}
		}
		if(!quiesce){
		...
		if(!ml.kingAttacked[player]){
			...
			recordMoves2(player, State4.PIECE_TYPE_KING, s.kings[player],
					kingMoves, ml, killer, s, m, quiesce);
		}
	}
	
	private void genMoves(final int player, State4 s, MoveList ml, ZMap m, boolean quiece){
		/*ml.upTakes[State4.PIECE_TYPE_KING] = s.pieces[1-player];
		ml.trades[State4.PIECE_TYPE_KING] = 0;
		...
		final long[] killer = quiece || ml.stackIndex-2 < 0? null: stack[ml.stackIndex-2].killer;
		
		if(ml.kingAttacked[player]){
			long kingMoves = State4.getKingMoves(player, s.pieces, s.kings[player]);
			recordMoves(player, State4.PIECE_TYPE_KING, s.kings[player], kingMoves, ml, killer, s, m, false);
		}
		
		long queens = s.queens[player];
		recordMoves(player, State4.PIECE_TYPE_QUEEN, queens,
				State4.getQueenMoves(player, s.pieces, queens), ml, killer, s, m, quiece);
		queens &= queens-1;
		recordMoves(player, State4.PIECE_TYPE_QUEEN, queens,
				State4.getQueenMoves(player, s.pieces, queens), ml, killer, s, m, quiece);
		queens &= queens-1;
		...
		recordMoves(player, State4.PIECE_TYPE_ROOK, rooks,
				State4.getRookMoves(player, s.pieces, rooks), ml, killer, s, m, quiece);
		rooks &= rooks-1;
		recordMoves(player, State4.PIECE_TYPE_ROOK, rooks,
				State4.getRookMoves(player, s.pieces, rooks), ml, killer, s, m, quiece);
		...
		recordMoves(player, State4.PIECE_TYPE_KNIGHT, knights,
				State4.getKnightMoves(player, s.pieces, knights), ml, killer, s, m, quiece);
		knights &= knights-1;
		recordMoves(player, State4.PIECE_TYPE_KNIGHT, knights,
				State4.getKnightMoves(player, s.pieces, knights), ml, killer, s, m, quiece);
		...
		recordMoves(player, State4.PIECE_TYPE_BISHOP, bishops,
				State4.getBishopMoves(player, s.pieces, bishops), ml, killer, s, m, quiece);
		bishops &= bishops-1;
		recordMoves(player, State4.PIECE_TYPE_BISHOP, bishops,
				State4.getBishopMoves(player, s.pieces, bishops), ml, killer, s, m, quiece);
		...
		if(!ml.kingAttacked[player]){
			long kingMoves = State4.getKingMoves(player, s.pieces, s.kings[player])|State4.getCastleMoves(player, s);
			recordMoves(player, State4.PIECE_TYPE_KING, s.kings[player], kingMoves, ml, killer, s, m, quiece);
		}
	}
