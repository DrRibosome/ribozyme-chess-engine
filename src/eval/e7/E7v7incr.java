...
/** E7, but with incremental mobility updating*/
public final class E7v7incr implements Evaluator2{
	...
	/** aggregates moves to determine which mobility scores need to be regenerated*/
	private long moveMaskAgg;
	/** raw move masks calculated from mobility, indexed [player][piece-type]*/
	private final long[][] pieceMoveMasks = new long[2][7];
	/** mobility scores, indexed [player][piece-type]*/
	private final int[][] cachedMobilityScores = new int[2][7];
	...
	/** mask determining wheter piece mobility should be recalculated, indexed [player][piece-type]*/
	private final long[][] mobilityRecalcMask = new long[2][7];
	...
	@Override
	public int lazyEval(final State4 state, final int player) {
	...
	private int score(final int player, final State4 s){
		...
		score += scoreMobility(player, s, clutterMult, nonPawnMaterial, attackMask[player],
				mobilityRecalcMask[player], moveMaskAgg, cachedMobilityScores[player], kingMoved[player]);
	...
	private void update(final long encoding, final boolean undo){
		final long pos1 = 1L << MoveEncoder.getPos1(encoding);
		final long pos2 = 1L << MoveEncoder.getPos2(encoding);
		moveMaskAgg |= pos1 | pos2;
		...
		if(taken != 0){
			...
			//mobilityRecalcMask[1-player][taken] = ~0; //flag taken piece to be recalculated
			moveMaskAgg = ~0; //less selective, but might be faster
		} else if(MoveEncoder.isEnPassanteTake(encoding) != 0){
			final long takePos = player == 0? pos2 >>> 8: pos2 << 8;
			moveMaskAgg |= takePos;
		...
		if(MoveEncoder.isPawnPromoted(encoding)){
			...
			mobilityRecalcMask[player][State4.PIECE_TYPE_QUEEN] = ~0; //flag queens to be recalculated
		}
		...
		if(MoveEncoder.isCastle(encoding) != 0){
			final int loc = MoveEncoder.getPos2(encoding);
			switch(loc){
			case 6:
				moveMaskAgg |= 1L<<7;
				moveMaskAgg |= 1L<<5;
	...
	private static int scoreMobility(final int player, final State4 s,
			final double clutterMult, final int[] nonPawnMaterial,
			final long[] attackMask, final long[] mobRecalcMask,
			final long moveMaskAgg, final int[] cachedMobilityScores,
			final boolean kingMoved){
		int mobScore = 0;
		...
		long aggAttackMask = 0;
		long enemyAttacks = enemyPawnAttacks;
		final long agg = s.pieces[0] | s.pieces[1];
		final long allied = s.pieces[player];
		
		//final long recalcMask = moveMaskAgg | (enemyPawnAttacks & ~agg);

		/*final boolean recalc = (moveMaskAgg & mobRecalcMask[State4.PIECE_TYPE_BISHOP]) != 0;
		final long initialAttackMask = attackMask[State4.PIECE_TYPE_BISHOP];
		final long initialMoveMask = mobRecalcMask[State4.PIECE_TYPE_BISHOP];*/
		if((moveMaskAgg & mobRecalcMask[State4.PIECE_TYPE_BISHOP]) != 0){
			int bishopScore = 0;
			long bishopAttackMask = 0;
			long bishopMoveMask = 0;
			for(long bishops = s.bishops[player]; bishops != 0; bishops &= bishops-1){
				...
				final long moves = rawMoves & ~allied & ~enemyAttacks;
				final int count = (int)BitUtil.getSetBits(moves);
				bishopScore += mobilityWeights[State4.PIECE_TYPE_BISHOP][count];
				bishopAttackMask |= moves;
				bishopMoveMask |= rawMoves | b;
			}
			mobRecalcMask[State4.PIECE_TYPE_BISHOP] = bishopMoveMask;
			attackMask[State4.PIECE_TYPE_BISHOP] = bishopAttackMask;
			cachedMobilityScores[State4.PIECE_TYPE_BISHOP] = bishopScore;
			mobScore += bishopScore;
		}
		mobScore += multWeight(cachedMobilityScores[State4.PIECE_TYPE_BISHOP], 1);
		aggAttackMask |= attackMask[State4.PIECE_TYPE_BISHOP];
		
		/*if(!recalc && (initialAttackMask != attackMask[State4.PIECE_TYPE_BISHOP])){
			System.out.println("failure");
			System.out.println("player = "+player);
			System.out.println(s);
			System.out.println("recalc = "+recalc);
			System.out.println("move mask agg:\n"+Masks.getString(moveMaskAgg)+"\n");
			System.out.println("initial attack mask:\n"+Masks.getString(initialAttackMask)+"\n");
			System.out.println("initial move mask:\n"+Masks.getString(initialMoveMask)+"\n");
			System.out.println("calculated attack mask:\n"+Masks.getString(attackMask[State4.PIECE_TYPE_BISHOP])+"\n");
			System.out.println("calculated move mask:\n"+Masks.getString(mobRecalcMask[State4.PIECE_TYPE_BISHOP])+"\n");
		}*/
		//assert recalc || (initialAttackMask == attackMask[State4.PIECE_TYPE_BISHOP]);

		if((moveMaskAgg & mobRecalcMask[State4.PIECE_TYPE_KNIGHT]) != 0){
			int knightScore = 0;
			long knightAttackMask = 0;
			long knightMoveMask = 0;
			for(long knights = s.knights[player]; knights != 0; knights &= knights-1){
				...
				final long moves = rawMoves & ~allied & ~enemyAttacks;
				final int count = (int)BitUtil.getSetBits(moves);
				knightScore += mobilityWeights[State4.PIECE_TYPE_KNIGHT][count];
				knightAttackMask |= moves;
				knightMoveMask |= rawMoves | k;
			}
			mobRecalcMask[State4.PIECE_TYPE_KNIGHT] = knightMoveMask;
			attackMask[State4.PIECE_TYPE_KNIGHT] = knightAttackMask;
			cachedMobilityScores[State4.PIECE_TYPE_KNIGHT] = knightScore;
		}
		mobScore += multWeight(cachedMobilityScores[State4.PIECE_TYPE_KNIGHT], 1);
		aggAttackMask |= attackMask[State4.PIECE_TYPE_KNIGHT];
		
		if(1==1||(moveMaskAgg & mobRecalcMask[State4.PIECE_TYPE_ROOK]) != 0 | kingMoved){
			final long allPieces = s.pieces[0]|s.pieces[1];
			final long enemyPawns = s.pawns[1-player];
			...
			for(long rooks = s.rooks[player]; rooks != 0; rooks &= rooks-1){
				...
				final long moves = rawMoves & ~allied & ~enemyAttacks;
				final int count = (int)BitUtil.getSetBits(moves);
				rookScore += mobilityWeights[State4.PIECE_TYPE_ROOK][count];
				...
				if(row == alliedKingRow){
					...
					if(alliedKingRow == backRank && count <= 4){
				...
				rookMoveMask |= rawMoves | r;
			}
			attackMask[State4.PIECE_TYPE_ROOK] = rookAttackMask;
			mobRecalcMask[State4.PIECE_TYPE_ROOK] = rookMoveMask;
			cachedMobilityScores[State4.PIECE_TYPE_ROOK] = rookScore;
		}
		mobScore += multWeight(cachedMobilityScores[State4.PIECE_TYPE_ROOK], 1);
		aggAttackMask |= attackMask[State4.PIECE_TYPE_ROOK];

		if((moveMaskAgg & mobRecalcMask[State4.PIECE_TYPE_QUEEN]) != 0){
			int queenScore = 0;
			long queenAttackMask = 0;
			long queenMoveMask = 0;
			for(long queens = s.queens[player]; queens != 0; queens &= queens-1){
				...
				final long moves = rawMoves & ~allied & ~enemyAttacks;
				final int count = (int)BitUtil.getSetBits(moves);
				queenScore += mobilityWeights[State4.PIECE_TYPE_QUEEN][count];
				queenAttackMask |= moves;
				queenMoveMask |= rawMoves | q;
			}
			attackMask[State4.PIECE_TYPE_QUEEN] = queenAttackMask;
			mobRecalcMask[State4.PIECE_TYPE_QUEEN] = queenMoveMask;
			cachedMobilityScores[State4.PIECE_TYPE_QUEEN] = queenScore;
		}
		mobScore += multWeight(cachedMobilityScores[State4.PIECE_TYPE_QUEEN], 1);
		attackMask[State4.PIECE_TYPE_EMPTY] |= attackMask[State4.PIECE_TYPE_QUEEN];
		
		attackMask[State4.PIECE_TYPE_PAWN] = Masks.getRawPawnAttacks(player, s.pawns[player]) & ~s.pieces[player];
		aggAttackMask |= attackMask[State4.PIECE_TYPE_PAWN];
		
		attackMask[State4.PIECE_TYPE_EMPTY] = aggAttackMask;
