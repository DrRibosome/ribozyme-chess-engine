...
/** 
 * gives analysis for futility pruning
 */
public class AnalysisFutility {
	public static void main(String[] args) throws IOException{
		...
		while(dis.available() > 0){
			...
			if(!temp.inCheck && !temp.isCapture && !temp.isDangerous && !temp.isTTEMove &&
				...
				final int k = temp.currentBeta-temp.lazyEval;
		...
		final int moveCountToDisplay = 64;
		...
		for(int a = 0; a < 7; a++){
			...
			System.out.println("mean difference between current beta and lazy eval");
			for(int d = 0; d < meanDiff[0].length; d++){
				if(d <= depthToDisplay){
